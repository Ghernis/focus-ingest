...
INSERT INTO dbo.dim_resource (
  ...
  region, name, application, environment, business, cost_center, tags_json, valid_from
)
SELECT
  s.provider, s.global_resource_id, s.resource_type, s.account_sk, s.sub_account_sk, s.service_sk,
  s.region, s.name, s.application, s.environment, s.business, s.cost_center, s.tags_json, s.valid_from
FROM src s
WHERE NOT EXISTS (
