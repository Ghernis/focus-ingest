...
-- Optional columnstore index (Azure SQL S3+ / Premium / vCore only).
-- Must use dynamic SQL: static CREATE COLUMNSTORE fails at compile time on unsupported tiers.
...
BEGIN
  BEGIN TRY
    DECLARE @cci_sql NVARCHAR(MAX) = N'
    ...
    EXEC sp_executesql @cci_sql;
  END TRY
  BEGIN CATCH
    IF ERROR_NUMBER() = 10794 OR ERROR_MESSAGE() LIKE N'%COLUMNSTORE%'
